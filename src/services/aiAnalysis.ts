...
export class AIAnalysisService {
  async generateReport(input: EvaluationInput): Promise<AnalysisReport> {
    ...
    return {
      ...
      dimensions: this.generateDimensions(input, totalScore),
      keyIssues: this.generateKeyIssues(input, totalScore),
      conclusion: {
  ...
  private calculateScore(input: EvaluationInput): number {
    ...
    if (textLength < 500) baseScore -= 15;
    else if (textLength > 2000) baseScore += 10;
    
    // 根据文本类型调整
    switch (input.textType) {
      case 'literary_submission':
        baseScore -= 5; // 文学投稿要求更高
        break;
      case 'web_serial':
        baseScore += 5; // 网文相对宽容
        break;
  ...
  private getEvaluationGoalName(goal: string): string {
    ...
    return names[goal] || '综合质量';
  }

  private generateDimensions(input: EvaluationInput, totalScore: number) {
    const dimensions = [
    ...
    return dimensions.map(dim => {
      ...
      return {
        ...
        strengths: this.generateStrengths(dim.key),
        weaknesses: this.generateWeaknesses(dim.key)
      };
    });
  }
  ...
  private generateKeyIssues(input: EvaluationInput, totalScore: number) {
    const issues: Array<{
      id: string;
      title: string;
  ...
  private generateRationale(recommendation: string, score: number): string {
